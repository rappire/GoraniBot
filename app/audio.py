...
from app.utility import check_url, check_playlist
...
class Audio:
    ...
    async def push_queue(self, ctx, title):
        ...
        song = Song(url=info.get('url'), title=info.get('title'), duration=info.get('duration'),
            webpage_url=info.get('webpage_url'), thumbnail=thumbnail,requester = ctx.author.name, uploader=info.get('uploader'))
    ...
    async def play(self):
        if self.current_song is None:
            ...
            self.guild.voice_client.play(discord.FFmpegPCMAudio(
                self.current_song.url, before_options='-reconnect 1 -reconnect_streamed 1 -reconnect_delay_max 5'), after=lambda e: self.next_play())
