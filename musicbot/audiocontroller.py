...
class AudioController(object):
    ...
    async def play_song(self, song):
        ...
        self.guild.voice_client.play(discord.FFmpegPCMAudio(
            song.base_url, before_options='-reconnect 1 -reconnect_streamed 1 -reconnect_delay_max 5', executable="ffmpeg.exe"), after=lambda e: self.next_song(e))
    ...
    async def udisconnect(self):
        channel = self.bot.get_channel(self.guild.id)
        # print(dir(channel))
        # if "send" in dir(channel):
        #     await channel.send("아아악!")
        if self.do_ak:
            if self.ak:
                self.ak = False
                ...
                self.guild.voice_client.play(discord.FFmpegPCMAudio(source='AK.mp3'))
                self.ak = True
        else:
            await self.stop_player()
            await self.guild.voice_client.disconnect(force=True)
